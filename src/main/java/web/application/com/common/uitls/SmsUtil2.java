...
public class SmsUtil2 {
	...
	private static final Log LOG = LogFactory.getLog(SmsUtil2.class);
	...
	static final String signName = "助跑公司";
	static final String accessKeyId = "LTAIXHpKxzPCjTvy";
	static final String accessKeySecret = "Tr1vbYYQJGaNTkWlvBPDdVJmMGDbUu";
...
//			SendSmsResponse sendSmsResponse = acsClient.getAcsResponse(request);
	...
	public static void sendUpdatePassCode(String moblie, String code  ) throws Exception {
		...
		request.setSignName(signName);
		// 必填:短信模板-可在短信控制台中找到
		request.setTemplateCode("SMS_150180081"); // 正式模版
		// 可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
...
//			SendSmsResponse sendSmsResponse = acsClient.getAcsResponse(request);
	...
	public static void sendLoginCode(String moblie, String code  ) throws Exception {
		...
		request.setSignName(signName);
		// 必填:短信模板-可在短信控制台中找到
		request.setTemplateCode("SMS_150180083"); // 正式模版
		// 可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
		request.setTemplateParam("{\"code\":\""+code+"\"}");
		LOG.info("阿里大鱼设置请求参数------code :"+ code  );
		// 选填-上行短信扩展码(无特殊需求用户请忽略此字段)
